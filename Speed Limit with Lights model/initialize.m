...
dmax = 200/5280; % eigth of a mile
vmax = zeros(1, nb); %speed limit
vmax(1:60) = 25;
vmax(61:68) = 60;
...
%this is average block time but taking care of traffic lights too
notraffic_avg_block_time = ((L-dmax)./ vmax) + ... 
                (1./nbin(i2)).*(dmax./vmax)+ ...
                (1-(1./nbin(i2))).*((dmax)*(dmax - dmin)./(vmax*dmin));
graph = sparse(i1, i2, notraffic_avg_block_time);
...
%startmovingtime designates the time a car is supposed to start moving
%again after dropping off a passenger. I give the passenger 10 second to
%leave the car
